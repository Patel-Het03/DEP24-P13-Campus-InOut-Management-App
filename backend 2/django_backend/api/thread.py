    ...
    def run(self):
        ...
        try:
            while True:
                ...
                queryset_locations = Location.objects.filter(
                    is_present=True, automatic_exit_required=True
                )
                ...
                for each_location in serializer_location.data:
                    ...
                    queryset_status_table = StatusTable.objects.filter(
                        is_present=True, location_id=location_id
                    )
                    serializer_status_table = StatusTableSerializer(
                        queryset_status_table, many=True
                    )
                    for each_status in serializer_status_table.data:
                        ...
                        date_time_of_this_ticket = serializer_ticket_table.data[-1][
                            "date_time"
                        ]

                        _date_of_ticket = date_time_of_this_ticket.split("T")[0]

                        _time_of_ticket = ":".join(
                            (date_time_of_this_ticket.split("T")[-1])
                            .split(".")[0]
                            .split(":")[:-1]
                        )
